...
from videoanalytics.analytics.tools.scene import Scene
...
object_kinds = ['птица', 'человек', ]
tools: dict[int, Tool | tuple[int, int]] = dict()

tools[0] = Scene(colour=(0, 0, 0), thickness=1)
tools[1] = Coords(200, 100)
tools[2] = Coords(300, 100)
tools[3] = Coords(300, 200)
tools[4] = Coords(200, 200)
...
tools[7] = Segment(colour=(0, 0, 0), thickness=1, start=tools[3], end=tools[4], )
tools[8] = Segment(colour=(0, 0, 0), thickness=1, start=tools[4], end=tools[1], )
tools[9] = Area(colour=(0, 0, 255), thickness=2, components=[
    tools[5],
    tools[6],
    tools[7],
    tools[8],
...
tools[12] = Counter(colour=(0, 0, 0), thickness=1, start=0, step=1, )

tools = {k: v for k, v in tools.items() if not (isinstance(v, tuple) or isinstance(v, Coords))}
