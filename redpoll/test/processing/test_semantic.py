...
# def test_fills_optional_args_with_default_values(source, event_source_prefix):
#     ast = Analyzer(event_source_prefix + source).analyze()
...
def test_raise_on_invalid_event_args(source, err_msg, event_source_prefix):
    ...
    assert err.missing_required_event_arg() in str(error.value)
