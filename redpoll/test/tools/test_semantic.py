...
def test_raise_on_self_id_as_arg():
    source = """объекты:; инструменты:
    ...
    with pytest.raises(SemanticError) as error:
        _ = analyzer.analyze()
    assert err.self_id_as_arg() in str(error.value)


def test_raise_on_undefined_id_as_arg():
    source = """объекты:; инструменты:
        *л1: прямая, от=(20, 30), до=*т1;
    ; обработка:;"""
    analyzer = Analyzer(source)
...
def test_raise_on_missing_required_arg():
    source = """объекты:; инструменты:
        *л1: прямая, от=(20, 30);
    ; обработка:;"""
    analyzer = Analyzer(source)
...
def test_raise_on_arg_type_mismatch():
    source = """объекты:; инструменты:
    ...
    with pytest.raises(SemanticError) as error:
        _ = analyzer.analyze()
    assert err.arg_type_mismatch() in str(error.value)
...
def test_raise_on_undefined_id_as_part():
    source = """объекты:; инструменты:
...
def test_raise_on_point_as_part():
    source = """объекты:; инструменты:
        *т1: (40, 50);
        *з1: зона, состав=...(*т1);
    ; обработка:;"""
...
def test_raise_on_area_as_part():
    source = """объекты:; инструменты:
        *з1: зона, состав = ...(
...
def test_raise_on_counter_as_part():
    source = """объекты:; инструменты:
        *л1: линия, состав = ...(
            прямая, от = (20, 20), до = (20, 40);
